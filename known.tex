...
\begin{defn}
  Call a prime $p$ \emph{$k$-decomposable} if there exists a finite
  multiset of positive integers $C$ such that $p = S(C)$ and $P(C) =
  k$. Call a prime \emph{nonresidue-decomposable} if $p$ is
  $k$-decomposable for some $k$ that is a nonresidue modulo $p$.
\end{defn}
...
\begin{fact}
  ...
  odd prime. The monodromy pairing on $\Jac(B_{p^r})$ is isomorphic to
...
\begin{fact}
 Let $p$ be an odd prime, and let $r$ be an odd positive integer
 \begin{enumerate}
 \item Suppose $p$ is not congruent to $1\pmod 8$. Then $p$ is 
  nonresidue-decomposable.
 
 \item Suppose $p$ is congruent to $1\pmod 4$. Let $q$ be a prime
  with $p\equiv k\pmod q$ and $\frac{q^{2}{4}<p$. If $q$ is 
  $k$-decomposable, and $\left(\frac{q}{p}\right)=-1$, then $p^{r}$ is
  nonresidue-decomposable.
  
  In particular, if $\left(\frac{q}{p}\right)=-1$ for some prime $q
  \equiv 3 \pmod 4$ and $\frac{q^2}{4} < p$, then $p^{r}$ is
  nonresidue-decomposable.
...
\begin{proof_short}
 Note that $p^{r}\equiv k^{r}\pmod q$, which is a non residue. 
 The rest follows immediately from the proof for $r=1$.
\end{proof_short}
...
\begin{evidence}
  We can try and relate genus to the maximum order of elements on
  2-vertex connected graphs, and induct on the order of $H$.
\end{evidence}
